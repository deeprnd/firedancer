...
**Description:** Define payment and payout event fixtures for failed payments and payouts, normalize payment intent into structured fields, add deterministic processor and beneficiary fixtures, reject malformed money-movement requests.
...
**Description:** Define beneficiary allowlist, enforce rail/currency/amount/retry limits, check sanctions/KYC/AML/risk flags, return allow/deny/require_approval/require_more_evidence/escalate decisions with user-facing reasons.
...
**Description:** Define draft response schema and evidence packet with source event, processor record, and risk flags, make responses review-only and non-executing, attach evidence hashes to retry and escalation proposals.
...
**Description:** Add demo flow showing failed payout classification, safe retry to known beneficiary, blocked retry, approval-required case, evidence-required case, and escalation with audit and replay artifacts.
