# V5.9: Crypto Thesis To Guarded Spot Trade
...
### V5.9.S1: Crypto intent and spot instrument catalog
...
- V5.9.S1.T1: Define a bounded crypto trading intent schema with account,
  ...
  exclusions.
- V5.9.S1.T2: Normalize plain-English single-pair and multi-asset allocation
...
- V5.9.S1.T3: Define deterministic spot instrument fixtures with canonical
...
- V5.9.S1.T4: Keep asset identity distinct from network and token-contract
  identity so a ticker collision cannot select an unintended asset.
- V5.9.S1.T5: Include approved BTC, ETH, and stablecoin spot examples plus
...
### V5.9.S2: Account, market, and portfolio snapshot
...
### V5.9.S3: Fee-aware spot plan and trade tickets
...
- V5.9.S3.T1: Build a deterministic single-pair or allocation plan from the
  normalized intent, eligible instrument catalog, account snapshot, and quote
  snapshots.
- V5.9.S3.T2: Define crypto spot ticket lines with pair, venue, side, order
...
- V5.9.S3.T3: Apply decimal precision, tick size, minimum order size, and
  rounding rules deterministically; reject a line that becomes invalid after
  rounding.
- V5.9.S3.T4: Show the expected stablecoin/cash reserve and per-asset and total
  portfolio exposure after the proposed fill.
- V5.9.S3.T5: Compare only policy-approved venue quotes and present the chosen
  route and rejected routes with evidence. A comparison does not grant
  cross-venue placement authority.
- V5.9.S3.T6: Hash-bind every ticket to intent, catalog, account snapshot,
...
### V5.9.S4: Crypto spot capability and risk checks
...
- V5.9.S4.T1: Evaluate crypto spot orders through `tkpoly` using the existing
...
### V5.9.S5: Paper and approved sandbox order lifecycle
...
- V5.9.S5.T1: Keep paper mode as the default and require an explicitly enabled
  V1.6 signed adapter manifest for sandbox submission.
- V5.9.S5.T2: Bind placement to the exact validated ticket hash, policy
...
- V5.9.S5.T3: Support bounded market and limit spot-order submission, status
  read-back, partial-fill representation, cancellation proposal, and terminal
  fill or rejection state in paper/sandbox mode.
- V5.9.S5.T4: Reconcile read-back pair, side, quantity, price, fee, status,
  venue order id, and action id against the submitted ticket.
- V5.9.S5.T5: Update the captured portfolio view from accepted fill evidence;
  never infer a fill from submission success alone.
- V5.9.S5.T6: Reject duplicate action ids, expired or revoked approvals,
...
### V5.9.S6: Crypto trading demo, audit, and replay
...
- V5.9.S6.T1: Add one deterministic local demo for the example BTC/ETH/USDC
  intent using captured account, catalog, quote, model, tool, and adapter
  fixtures.
- V5.9.S6.T2: Show eligible assets and pairs, rejected alternatives, selected
  venue routes, fees, quote ages, tickets, policy outcomes, fills, and updated
  exposure.
- V5.9.S6.T3: Include blocked stale-quote, unsupported-pair,
  over-concentration, insufficient-balance, and direct-placement-bypass
  scenarios.
- V5.9.S6.T4: Emit append-only audit, metrics, diagnostics, and evidence for
...
- V5.9.S6.T5: Replay from captured source intent and substitutions with model,
  market, exchange, adapter, and transfer effects structurally disabled.
- V5.9.S6.T6: Report the first divergent intent, snapshot, quote, ticket,
