...
#define TK_THESIS_HASH_K1 ((uint64_t)TK_THESIS_SCHEMA_VERSION)
...
tk_thesis_input_hash( uint16_t              user_text_len,
  ...
  tk_siphash13_t * sip = tk_siphash13_init( _sip, TK_THESIS_HASH_K0, TK_THESIS_HASH_K1 );
  ...
  size_t theme_stride = TK_THESIS_MAX_CANONICAL_ID_LEN;
  for( uint8_t i = 0; i < theme_count; i++ ) {
    TK_HASH( themes_flat + (size_t)i * theme_stride, theme_stride );
  }
  ...
  size_t ref_stride = TK_THESIS_CLASSIFICATION_REF_STRIDE;
  for( uint8_t i = 0; i < sector_filter_count; i++ ) {
    TK_HASH( sector_filter_flat + (size_t)i * ref_stride, ref_stride );
  }
  TK_HASH( &industry_filter_count, sizeof(uint8_t) );
  for( uint8_t i = 0; i < industry_filter_count; i++ ) {
    TK_HASH( industry_filter_flat + (size_t)i * ref_stride, ref_stride );
  }
  ...
  size_t ticker_stride = TK_THESIS_MAX_TICKER_LEN;
  for( uint8_t i = 0; i < requested_ticker_count; i++ ) {
    TK_HASH( requested_tickers + (size_t)i * ticker_stride, ticker_stride );
  }
  ...
  TK_HASH( user_text,              (uint64_t)user_text_len );
...
#define TK_BASKET_HASH_K1 ((uint64_t)TK_BASKET_SCHEMA_VERSION)
...
tk_basket_hash( uint64_t         thesis_id,
  ...
  tk_siphash13_t _sip[1];
  tk_siphash13_t * sip = tk_siphash13_init( _sip, TK_BASKET_HASH_K0, TK_BASKET_HASH_K1 );
  ...
  size_t stride = TK_BASKET_MAX_TICKER_LEN;
  for( uint8_t i = 0; i < instrument_count; i++ ) {
    TK_HASH( tickers + (size_t)i * stride, stride          );
...
#define TK_TICKET_HASH_K1 ((uint64_t)TK_TRADE_TICKET_SCHEMA_VERSION)
...
tk_trade_ticket_hash( uint64_t         basket_id,
  ...
  tk_siphash13_t _sip[1];
  tk_siphash13_t * sip = tk_siphash13_init( _sip, TK_TICKET_HASH_K0, TK_TICKET_HASH_K1 );
  ...
  size_t stride = TK_TICKET_MAX_TICKER_LEN;
...
#define TK_PAPER_ORDER_HASH_K1 ((uint64_t)TK_TRADE_TICKET_SCHEMA_VERSION)
...
tk_paper_order_hash( uint64_t ticket_id,
  ...
  tk_siphash13_t _sip[1];
  tk_siphash13_t * sip = tk_siphash13_init( _sip, TK_PAPER_ORDER_HASH_K0, TK_PAPER_ORDER_HASH_K1 );
  ...
  size_t filled_stride = TK_TICKET_MAX_TICKER_LEN;
  for( uint8_t i = 0; i < filled_line_item_count; i++ ) {
    TK_HASH( filled_tickers + (size_t)i * filled_stride, filled_stride    );
  ...
  for( uint8_t i = 0; i < holding_count; i++ ) {
    TK_HASH( holding_tickers + (size_t)i * filled_stride, filled_stride    );
